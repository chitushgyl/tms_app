<template>
	<view class="add">
		<u-navbar :is-back="false" :border-bottom='false' title="编辑地址">
...
<script>
	...
	export default {
		...
		created() {
			...
			this.self_id = this.editor.self_id
			this.province = this.editor.sheng_name
			this.city = this.editor.shi_name
			this.area = this.editor.qu_name
			this.quiId = this.editor.self_id
			this.address = this.editor.address
			this.contacts = this.editor.contacts
			this.tel = this.editor.tel
		},
		methods: {
			// 省市区确认事件
			pickerChange(e) {
